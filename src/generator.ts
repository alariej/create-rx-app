...
const WINDOWS_TEMPORARY_KEY_PATH = path.join(
...
export class Generator {
  ...
  private buildSelfSignedCertificate(currentUser: string | undefined): string {
    ...
    console.log(
      '%s %s',
      chalk.blue.bold('[windows]'),
      chalk.white.bold('Generating self-signed certificate...'),
    );

    const certificateDestPath = path.join(
      projectPath,
      WINDOWS_FOLDER,
      projectName,
    );
    ...
    const { status, stdout } = spawnSync('powershell', [
      '-command',
      certificateArgs,
    ]);
    if (status === 0) {
    ...
    console.log(
      '%s %s',
      chalk.blue.bold('[windows]'),
      chalk.red.bold('Failed to generate Self-signed certificate.'),
    );
    console.log(
      '%s %s',
      chalk.blue.bold('[windows]'),
  ...
  private printInstructions(): void {
    ...
    const windowsProjectPath = `${path.resolve(
      projectPath,
      WINDOWS_FOLDER,
      projectName,
    )}.sln`;
    ...
    console.log(
      chalk.green.bold('%s was successfully created. \n'),
      projectName,
    );
    ...
    console.log(
      chalk.yellow.bold('To build Web production version of your app:'),
    );
    console.log('  cd %s', projectPath);
    ...
    console.log('  - or -');
    console.log(
      '  open %s project in Xcode',
      path.relative(projectPath, xcodeProjectPath),
    );
    ...
    console.log('  - or -');
    console.log(
      chalk.white.bold('  open %s project in Visual Studio'),
      windowsProjectPath,
    );
    console.log('  press the Run button \n');
  }
  ...
  private isBinary(srcPath: string): boolean {
    return (
      ['.png', '.jpg', '.jar', '.ico', '.pfx', '.keystore'].indexOf(
